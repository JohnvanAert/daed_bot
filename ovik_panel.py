...
from aiogram.types import (
    Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, FSInputFile, Document
)
...
async def handle_ovik_submit(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("📎 Прикрепите ZIP файл по ОВиК/ТС.")
    await callback.answer()

# 💾 Приём файла от ОВиК-специалиста
...
async def receive_ovik_file(message: Message, state: FSMContext, bot):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="❌ Исправить", callback_data=f"revise_ovik:{order_id}")
        ]
    ])
    ...
    await bot.send_document(
        ...
        caption=f"📩 Получен ZIP файл от специалиста по ОВиК по заказу <b>{order['title']}</b>",
        parse_mode="HTML",
        reply_markup=keyboard
    )

    await message.answer("✅ Файл по ОВиК отправлен ГИПу.")
    await state.clear()
