...
async def show_gs_tasks(message: Message):
    ...
    if not tasks:
        await message.answer("📭 У вас пока нет задач по ГС.")
        return
    ...
    for task in tasks:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📤 Отправить на проверку", callback_data=f"submit_gs_task:{task['task_executor_id']}")]
        ])
...
@router.callback_query(F.data.startswith("submit_gs_task:"))
async def handle_gs_submit_task(callback: CallbackQuery, state: FSMContext):
    task_executor_id = int(callback.data.split(":")[1])
    await state.update_data(task_executor_id=task_executor_id)
    await state.set_state(SubmitGSTaskFSM.waiting_for_submission_file)
    await callback.message.answer("📎 Пришлите ZIP-файл для отправки специалисту по ГС.")
    await callback.answer()


@router.message(SubmitGSTaskFSM.waiting_for_submission_file, F.document)
async def handle_gs_submission_file(message: Message, state: FSMContext, bot: Bot):
    ...
    filename = f"gs_submit_{task_executor_id}_{document.file_name}"
    ...
    await message.answer("✅ Задание отправлено специалисту по ГС.")
    await state.clear()
